...
The system SHALL provide a `/llmw-init` command that bootstraps the wiki structure and installs all LLM Wiki skills into the project.

#### Scenario: New project initialized
- **WHEN** a user runs `/llmw-init` in a project without wiki structure
- **THEN** the skill creates `raw/` and `wiki/` directories, installs all 4 skill files under `.opencode/skills/llmw-*` and 4 command files under `.opencode/commands/`, and adds wiki conventions to `AGENTS.md`

#### Scenario: Existing project re-initialized
- **WHEN** a user runs `/llmw-init` in a project that already has wiki structure
- **THEN** the skill detects existing directories and skills, only adds missing pieces, and does not overwrite existing wiki content
